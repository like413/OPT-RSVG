...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        return output
class TransformerDINODecoder(nn.Module):
    ...
    def forward(self, tgt, memory,text,
        ...
        for layer in self.layers:
            output = layer(output, memory,text, tgt_mask=tgt_mask,
                           memory_mask=memory_mask,
                           tgt_key_padding_mask=tgt_key_padding_mask,
                           memory_key_padding_mask=memory_key_padding_mask,
                           pos=pos, query_pos=query_pos,text_pos=text_pos)
            if self.return_intermediate:
                intermediate.append(self.norm(output))
...
class TransformerDINODecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,text,
        ...
        tgt2 = self.multihead_attn2(query=self.with_pos_embed(tgt, query_pos),
                                   ...
                                   value=text, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]
        tgt = tgt + self.dropout3(tgt2)
        tgt = self.norm3(tgt)
    ...
    def forward_pre(self, tgt, memory,text,
        ...
        tgt3 = self.multihead_attn2(query=self.with_pos_embed(tgt2, query_pos),
                                   ...
                                   value=text, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]
        tgt23= tgt + self.dropout2(tgt3)
        tgt3 = self.norm3(tgt2)
    ...
    def forward(self, tgt, memory,text,
                ...
                text_pos: Optional[Tensor] = None):
        if self.normalize_before:
            return self.forward_pre(tgt, memory,text,tgt_mask, memory_mask,
                                    tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos,text_pos)
        return self.forward_post(tgt, memory,text,tgt_mask, memory_mask,
                                    tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos,text_pos)
